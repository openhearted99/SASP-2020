...
#5 - BHs
    ...
    def process_lightcone(self,minz=0.0,maxz=20.0):
        ...
        for i,cyl in enumerate(self.cylinder_number):
            ...
            # Load the data
            fields=['SubhaloMass','SubhaloMassInMaxRad','SubhaloMassInRadType','SubhaloMassInMaxRadType',
                    'SubhaloPos','SubhaloSFR','SubhaloSFRinRad','SubhaloVel',
                    'SubhaloBHMass','SubhaloBHMdot','SubhaloStellarPhotometrics','SubhaloWindMass']
            ...
            #need to define SubFindID before filtering
            N= gmag.shape[0]
            subhalos['SubFindID']=np.arange(N)

            print('Before filtering: ', subhalos['SubFindID'].shape)
            ...
            print('After filtering: ', subhalos['SubFindID'].shape)
            
            # Now, periodicize
            subhalos = self.periodicize(subhalos,self.L_comovingh*1000.0)
            
            
            print('After periodicize: ', subhalos['SubFindID'].shape)
    ...
    def periodicize(self,subhalos,boxL):
        ...
        #need to bring SubFindID definition up in the code to before filtering occurs
...
class cylinder_catalog:
    def __init__(self,snapnum,subhalos,ci,RA_deg,DEC_deg,snapz,galaxy_camera_posx,galaxy_camera_posy,galaxy_camera_posz,centerz,galaxy_camera_velx,galaxy_camera_vely,galaxy_camera_velz,cyl):
        ...
        for i,index in enumerate(ci):
            self.cosmological_redshift[i] = np.float64(z_at_value(WMAP7.comoving_distance,
                                                                  self.galaxy_comoving_z_mpc[i]*u.megaparsec,ztol=1e-12,maxfun=2000))
        ...
        self.mstar_msun = subhalos['SubhaloMassInRadType'][ci,4]*(1.0e10)/ilh
        self.mgas_msun = subhalos['SubhaloMassInRadType'][ci,0]*(1.0e10)/ilh #includes wind mass
        self.mbh_msun = subhalos['SubhaloMassInRadType'][ci,5]*(1.0e10)/ilh
        self.mhalo_msun = subhalos['SubhaloMass'][ci]*(1.0e10)/ilh

        self.baryonmass_msun = self.mstar_msun + self.mgas_msun + self.mbh_msun #within 2x stellar half mass radius... best?

        self.xpos_ckh = subhalos['SubhaloPos'][ci,0] #in cKpc/h of max bound part
        self.ypos_ckh = subhalos['SubhaloPos'][ci,1]
        self.zpos_ckh = subhalos['SubhaloPos'][ci,2]
        ...
        self.xvel_kms = subhalos['SubhaloVel'][ci,0]
        self.yvel_kms = subhalos['SubhaloVel'][ci,1]
        self.zvel_kms = subhalos['SubhaloVel'][ci,2]

        self.sfr = subhalos['SubhaloSFRinRad'][ci]
        self.bhmdot = subhalos['SubhaloBHMdot'][ci]

        self.gmag = subhalos['SubhaloStellarPhotometrics'][ci,4]
        self.rmag = subhalos['SubhaloStellarPhotometrics'][ci,5]
        self.imag = subhalos['SubhaloStellarPhotometrics'][ci,6]
        self.zmag = subhalos['SubhaloStellarPhotometrics'][ci,7]

        self.gmag_apparent=subhalos['gmag'][ci]
...
if __name__=="__main__":
    ...
    catalog_xyz = process_lightcone_catalog(lightcone="./lightcone_geometry/tng300_6_5_zyx.txt",
                                            base_dir='/home/tnguser/sims.TNG/TNG300-1/output/',mag_limit=magl)
    catalog_xyz = catalog_xyz.process_lightcone(minz=minz,maxz=maxz)
    catalog_xyz.output_catalog('./results/Lightcone_TNG300-1_mag30_6_5_zyx.txt')
